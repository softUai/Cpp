/*
Faça um Programa que pergunte quanto você ganha por hora e o número de horas trabalhadas no mês. Calcule e mostre 
o total do seu salário no referido mês, sabendo-se que são descontados 11% para o Imposto de Renda, 8% para o INSS 
e 5% para o sindicato, faça um programa que nos dê:
salário bruto.
quanto pagou ao INSS.
quanto pagou ao sindicato.
o salário líquido.
calcule os descontos e o salário líquido, conforme a tabela abaixo:
+ Salário Bruto : R$
- IR (11%) : R$
- INSS (8%) : R$
- Sindicato ( 5%) : R$
= Salário Liquido : R$
*/
#include <iostream>

using namespace std;

int main () {
	float salarioBruto, IR = 0.11, INSS = 0.08, sind = 0.05;
	cin >> salarioBruto;
	cout << " + Salário Bruto : R$ " << salarioBruto << endl;
	cout << " - IR (11%) : R$ " << salarioBruto*IR << endl;
	cout << " - INSS (8%) : R$ " << salarioBruto*INSS << endl;
	cout << " - Sindicato ( 5%) : R$ " << salarioBruto*sind << endl;
	cout << " = Salário Liquido : R$ " << salarioBruto - (salarioBruto*(IR + INSS + sind)) << endl;
	return 0;
}